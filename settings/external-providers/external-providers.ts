...
import { editExternalProviders } from './inline-providers/edit_form_providers.service'; //Inline Editing External Providers
...
@Component({
  ...
  template: require('./external-providers.html'),
  styles: [require('./external-providers.scss')],
...
export class ExternalProviders {
    ...
    constructor(
    public _data_pagination: DataPagination, public http: Http, public params: RouteParams, public router: Router, 
    public _titleService: TitleService, public _pag: myPagination, private ngZone: NgZone, public location : Location , 
    public modal: Modal, public load: LoadingGif, viewContainer: ViewContainerRef, public _providers_service: DataPropertiesProviders, public _edit_prov: editExternalProviders) {
    ...
    changeMyTitle() {
        this._titleService.change('External Providers');
...
ngOnInit() { 
    ...
    if ( this.location.path() == '/app/settings/external-providers') {
        this.restore_search();
    }
    ...
    f.submit(function(e){
        e.preventDefault();
        m.html('Search For External Provider').addClass('show');
        x.html('Search For External Provider').addClass('show');
        f.addClass('explode');
        setTimeout(function(){
            s.val('');
    ...
    if ( this.params.get('cp') ) {
        var letter = this._providers_service.search_ext_provider; //Store letter
        setTimeout(()=>{
            this._providers_service.get_external_providers(letter,{ page : this.params.get('cp') })
            .map(data => 
            {    
    ...
    searchLetters.subscribe((val : any) => {
        if (this._providers_service._data_pagination.total_page == 0 ){
            if (val.which == 8){
...
current_page_change(data){
        this._providers_service.get_external_providers(this._providers_service.search_ext_provider, { page: data.selectedPage , type : data.type})
        .map(json_response =>
...
get_size(){
  ...
  if(viewport_width < 1200){
    this.view_port_width_prov = false;
  } else if(viewport_width > 1200) {
    this.view_port_width_prov = true;
  }
}
...
/// Select All (Checkbox) ///
select_all(){
    $('.check-table-prov:checked').attr('checked', true);
    var inputs = jQuery('.check-table-prov');
...
enabled_disabled_all(){       
    this._providers_service.toggle_status = !this._providers_service.toggle_status; //If true change to false, If false change to true boolean
    var status = !this._providers_service.toggle_status;
    this._providers_service.list_id = []; //Clean array
    var inputs = jQuery('.check-table-prov');
    for(var i=0; i<inputs.length; i++){
        var checked = $('#checkbox' + i).is(":checked"); 
        if(checked == true){
            var id_content = $('#checkbox' + i).val();
            this._providers_service.list_id.push(id_content);
        }
    }
    console.log('Array de id providers' + this._providers_service.list_id);
    var multiple_id=this._providers_service.list_id;
    this.enabled_disabled_toggles(multiple_id, this._providers_service.list_id, status)
    .map(json_response => 
...
enabled_disabled_toogle(id, status){
    this._providers_service.list_id = []; //Clean array
    this._providers_service.list_id.push(id);
    var single_id= this._providers_service.list_id;
    this._providers_service.list_id = []; //Clean array
    var inputs = jQuery('.check-table-prov');
    for(var i=0; i<inputs.length; i++){
        var checked = $('#checkbox' + i).is(":checked"); 
    ...
    console.log('Array de id providers' + this._providers_service.list_id);
    this.enabled_disabled_toggles(single_id, this._providers_service.list_id, status)
    .map(json_response => 
...
enabled_disabled_toggles(id, selected_checkboxs, status){   
    ...
    .map( 
        response => {           
            this._providers_service.get_external_providers(letter,{ page : this._providers_service.current_page })
            .subscribe((json_response) => {
                ...
                return this._providers_service.list_id; //Send back response to the call of the method to use as event_type variable     
        }, error => {}
    ); 
}
...
search_ext_providers() {
    this._providers_service.current_page=1;
    this._providers_service.firstof_page=1;
    var letter = this._providers_service.search_ext_provider; //Store letter
    this._providers_service.get_external_providers(letter,{ page : 1})
    .map(data => 
    {    
        if ( data.numbers_of_pages > 1 ) {
...
*/
restore_search(){
    this._providers_service.search_ext_provider = '';
    var letter = this._providers_service.search_ext_provider; //Store letter
    this.search_ext_providers();

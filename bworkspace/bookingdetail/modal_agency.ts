...
export class ModalAgencyBooking {
...
constructor(public http: Http, public modal: Modal, viewContainer: ViewContainerRef, public _loc: Location, public _rol: RolloverAutocompletes,
            myElement: ElementRef, public load: LoadingGif, public _properties: DataPropertiesBookings,  public _filters: filter,
...
load_agency_data() { 
  let get_success;
  let url = myGlobals.host+'/api/admin/booking_workspace/booking_detail/edit/agency/get_agency'; 
  let body=JSON.stringify({ id_booking: this._properties.id_bookings});
  console.log(body);
        ...
        .subscribe( 
          response => {
            ...
            if(get_success == true){
              ...
              this._properties.record_locator = response.json().record_locator;
...
get_list_agencies(letter) {
  ...
  let url = myGlobals.host+'/api/admin//booking_workspace/booking_detail/edit/agency/get_agencies_autocomplete';
...
get_list_operators(letter) {
  ...
  let url = myGlobals.host+'/api/admin//booking_workspace/booking_detail/edit/agency/get_operators_autocomplete';
...
get_list_locators(letterOrNumber) {
  ...
  let url = myGlobals.host+'/api/admin//booking_workspace/booking_detail/edit/agency/get_locators_autocomplete';
...
filter_locator() {
  if (this.recordlocator !== ""){
      var letterOrNumber = this.recordlocator; //Store Letter or Number
...
select_locator(itemLoc){
    this.recordlocator = itemLoc; 
    this._properties.record_locator = itemLoc; 
    this.filteredListLoc = [];
}
...
save_all_data(send_mail, e) {
  ...
  let url = myGlobals.host+'/api/admin//booking_workspace/booking_detail/edit/agency/save';
  let body=JSON.stringify({ record_locator_booking: this._properties.record_locator_actual, agency_code: this.agencyCode, record_locator_new: this.recordlocator, operator_code: this.operatorCode, is_new_locator: this.is_new_locator, send_mail: send_mail });
        ...
        if(updated == true){
          if(this.is_new_locator == true){
            new_locator_generated = response.json().new_locator_generated;
             ...
             if(this.recordlocator==''){ //In case of empty Locator put default locator
              new_locator_generated = this.recordlocator;
             } else{ //New Locator added manually
              new_locator_generated = this._properties.id_bookings;
             }
          }
          this._properties.get_booking_detail(this._properties.id_bookings);        
          ...
          this._loc.go('/app/bworkspace/bookingdetail;id_bookings='+this._properties.id_bookings+';'+ get_url);
...
enabled_locator(){ //Button radio "Use an Existing Locator"
  this._properties.record_locator = this.recordlocator; //Store default Locator
  ...
  this.recordlocator =  this._properties.record_locator; //Store backup Locator
...
remove_modal_agency(){
  this._properties.record_locator = this.recordlocator; //Refresh data for field Record Locator  
  jQuery("modal-backdrop").hide();
